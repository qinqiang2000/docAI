...
def load_file_label_data(file_path):
    ...
    if session['label_data'] is not None:
        # 找到与当前文件名匹配的行
        matched_rows = session['label_data'][session['label_data']['filename'] == filename]
        if not matched_rows.empty:
            # 如果找到匹配的行，将这些行的数据添加到data_str_list中
            matched_rows = matched_rows.drop(columns=['filename', 'page']).to_dict('records')
...
def save_labels(labeled_data, filename, dataset_name, persist=True):
    ...
    if 'label_data' in st.session_state and session['label_data'] is not None:
        ...
        session['label_data'] = session['label_data'].loc[
            ~((session['label_data']['filename'] == filename) & (session['label_data']['page'].isin(new_data['page'])))]
        # 然后添加新的标签数据到最前面
...
with expander:
    if 'label_data' in st.session_state and session['label_data'] is not None:
        ...
        cols.remove('page')
        # 将'filename'和'page'添加到列名列表的前面
        cols = ['filename', 'page'] + cols
