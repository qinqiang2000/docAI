...
sys_prompt_img = """根据给出图片，提取银行回单的关键字段，并以JSON格式输出。 """
sys_prompt_md = """根据给出Markdown文本，提取银行回单的关键字段，并以JSON格式输出。 """
sys_prompt = """
注意：
- 如果字段没值，取空字符串为值。
...
def save_excel(root, filename, ret):
    # 设置文件名为 "a.xlsx"
    excel_file = os.path.join(root, "gpt4omini_textin.xlsx")
...
def batch_banks(root_dir):
    ...
    for filename in os.listdir(root_dir):
        ...
        if os.path.isfile(fp) and filename.lower().endswith('.png'):
            str = LLMOpenAI(azure_model, azure_client).generate_text("", sys_prompt_img+sys_prompt, fp)
...
def batch_banks_md(root_dir):
    # 列出 root_dir 目录下的所有文件和文件夹
    for filename in os.listdir(root_dir):
        # 获取当前文件的完整路径
        fp = os.path.join(root_dir, filename)

        if os.path.isfile(fp) and filename.lower().endswith('.md'):
            # 打开文件并读取内容
            with open(fp, 'r', encoding='utf-8') as file:
                text = file.read()
                print(f"text: \n {text}")

            str = LLMOpenAI(azure_model, azure_client).generate_text(sys_prompt_md+sys_prompt + '\n' + text, "")
            ...
            save_excel(root_dir, filename, ret)
# 示例调用
batch_banks("/Users/qinqiang02/job/客户/索菲亚电子档案/银行回单测试_banks/img/tmp")
# batch_banks_md("/Users/qinqiang02/job/客户/索菲亚电子档案/银行回单测试_banks/markdown_textin/b")
# print(sys_prompt_md+sys_prompt)